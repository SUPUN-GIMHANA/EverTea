...
export default function GrowthChart({ data, status }: GrowthChartProps) {
  ...
  const getBarProps = () => {
    ...
    const availableWidth = CHART_WIDTH - (PADDING * 3); // Space between circles
        ...
        <View style={styles.chartContainer}>
          <Svg width={CHART_WIDTH} height={CHART_HEIGHT} style={styles.chart}>
            ...
            <Line
              ...
              stroke={status === "Good" ? "#20C58D" : "#FF6666"}
              strokeWidth="4"
            />
            ...
            {bars.map((bar, index) => (
              <Rect
                ...
                width={bar.width}
                height={bar.height}
                fill={status === "Good" ? "#20C58D" : "#FF6666"}
                rx={0}
              />
            ))}
            ...
            <Circle
              cx={PADDING}
              cy={CHART_HEIGHT - PADDING}
              r="4"
              fill="#fff"
              stroke={status === "Good" ? "#4caf50" : "#ff7043"}
              strokeWidth="1.5"
            />
            <Circle
